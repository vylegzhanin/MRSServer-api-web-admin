...
class TestPanelsViewUI : UI() {
    ...
    private object Demo {
        ...
        fun getChart(): Component {
            ...
            configuration.chart.zoomType = ZoomType.X
            configuration.chart.spacingRight = 20

            configuration.title.text = "USD to EUR exchange rate from 2006 through 2008"

            val title = if (Page.getCurrent().webBrowser.isTouchDevice)
                "Drag your finger over the plot to zoom in"
            else
                "Click and drag in the plot area to zoom in"
            configuration.subTitle.text = title

            configuration.getxAxis().type = AxisType.DATETIME
            configuration.getxAxis().minRange = TWO_WEEKS
            configuration.getxAxis().title = AxisTitle("")

            configuration.legend.enabled = false

            val yAxis = configuration.getyAxis()
            yAxis.title = AxisTitle("Exchange rate")
            yAxis.min = 0.6
            yAxis.startOnTick = false
            yAxis.showFirstLabel = false

            configuration.tooltip.shared = true

            val plotOptions = PlotOptionsArea()

            val fillColor = GradientColor.createLinear(0.0, 0.0, 0.0, 1.0)
            fillColor.addColorStop(0.0, SolidColor.LIGHTBLUE)
            fillColor.addColorStop(1.0, SolidColor.DARKBLUE)
            plotOptions.fillColor = fillColor

            plotOptions.lineWidth = 1
            plotOptions.shadow = false

            val marker = Marker()
            marker.enabled = false
            val hoverState = Hover(true)
            hoverState.radius = 5
            val states = States()
            states.hover = hoverState
            marker.states = states

            val hoverStateForArea = Hover(true)
            hoverStateForArea.lineWidth = 1
            val statesForArea = States()
            statesForArea.hover = hoverStateForArea
            plotOptions.states = statesForArea
            plotOptions.marker = marker
            plotOptions.shadow = false
            ...
            val ls = ListSeries()
            val options = PlotOptionsArea()
            options.pointInterval = DAY_IN_MILLIS
            ls.plotOptions = options
            ls.name = "USD to EUR"

            try {
                options.pointStart = df.parse("2006/01/02").time
